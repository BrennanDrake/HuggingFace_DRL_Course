...
if __name__ == "__main__":
    ...
    # Vector-compatible wrappers:
    envs = gym.wrappers.vector.RecordEpisodeStatistics(envs)
    envs = gym.wrappers.vector.RecordVideo(envs, **video_kwargs, fps=30)
    ...
    for _ in range(20000):
        ...
        # For vector envs, these are arrays (shape [num_envs])
        done = terminated | truncated

        # Episode stats live in info; for vector wrapper it’s typically under:
        # info["episode"]["r"] and info["episode"]["l"] as arrays (one per env)
        if "episode" in info:
            # print all finished episodes this step
            for i, ep_ret in enumerate(info["episode"]["r"]):
                # Some entries may be 0/empty if that env didn't finish this step; you can guard if desired
                print(f"env {i} episodic return {ep_ret}")
